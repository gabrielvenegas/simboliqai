...
import { generateTextSVG } from "@/lib/font-helpers/font-to-path";
...
export async function generateLogo(payload: {
  ...
  if (payload.devMode) {
    const combinedSvg = {
      ...
      text: '<text style="font-family: "Inter", sans-serif; fill: black;">subtracker</text>',
      font: "",
    };

    // TESTE
    const interFont = fontsMap.professional.find(
      ({ name }) => name === "Inter",
    );

    if (interFont) {
      const fontResponse = await fetch(
        `${process.env.GOOGLE_FONTS_URL}?key=${process.env.GOOGLE_FONTS_API_KEY}&family=Inter`,
      );

      const webfont = await fontResponse.json();
      const url = webfont.items[0].files.regular;
      const fontBuffer = fetch(url).then((res) => res.arrayBuffer());

      const font = opentype.parse(await fontBuffer);
      const textSvg = generateTextSVG(font, "subtracker", 40, {
        fill: "#000",
        strokeWidth: "1px",
      });

      const textPathsOnly = textSvg
        .replace(/<\?xml.*?>/g, "")
        .replace(/<!DOCTYPE.*?>/g, "")
        .replace(/<svg[^>]*>/, "")
        .replace(/<\/svg>/, "");
      ...
      return {
        success: true,
        svg: { ...combinedSvg, text: textPathsOnly },
        fontFamily: "",
      };
    }

    return { success: false, svg: {} as any, fontFamily: "" };
  }
