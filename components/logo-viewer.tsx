...
export default function LogoViewer({
  ...
  // step 1: Setup with wide viewBox
  useEffect(() => {
  ...
  `;
    ...
    const textGroup = svg.querySelector("#svgGroup") as SVGGElement;
    if (!iconGroup || !textGroup) return;

    const iconBox = iconGroup.getBBox();
    const textBox = textGroup.getBBox();
    ...
    iconGroup.setAttribute(
      "transform",
      `scale(${scaleFactor}) translate(${xShift}, 0)`,
    );
  ...
  // step 2: Measure and set tight 1:1 viewBox
  useEffect(() => {
    ...
    const innerContainer = svg.querySelector("#inner-container") as SVGGElement;
    if (!innerContainer) return;

    const innerBox = innerContainer.getBBox();

    // Shift innerContainer to (0,0)
    const xShift = -innerBox.x;
    const yShift = -innerBox.y;
    innerContainer.setAttribute("transform", `translate(${xShift}, ${yShift})`);
